...
Static Function Importa()
	...
	For i:=1 to Len(aDados)
		...
		oProcess:IncRegua1("Importando Ativo Fixo..."+Alltrim(aDados[i,1])+Alltrim(aDados[i,2])+"-"+chr(13)+chr(10)+Alltrim(aDados[i,4]))
		...
		If !dbSeek(xFilial("SN1")+aDados[i,1]+ Padl(aDados[i,2],4,"0"))
			...
			AAdd(aCab,{"N1_ITEM" , Padl(aDados[i,2],4,"0") ,NIL})
			AAdd(aCab,{"N1_AQUISIC", StoD(Substr(aDados[i,3],7,4)+Substr(aDados[i,3],4,2)+Substr(aDados[i,3],1,2)) ,NIL})
			AAdd(aCab,{"N1_DESCRIC", Substr(aDados[i,4],1,40) ,NIL})
			AAdd(aCab,{"N1_QUANTD" , 1 ,NIL})
			AAdd(aCab,{"N1_CHAPA" , "" ,NIL})
			AAdd(aCab,{"N1_PATRIM" ,"" ,NIL})
			AAdd(aCab,{"N1_NFISCAL" ,"" ,NIL})
			...
			{"N3_ITEM" , Padl(aDados[i,2],4,"0") ,NIL},;
			{"N3_TIPO" , "01" ,NIL},;
			{"N3_BAIXA" , "0" ,NIL},;
			...
			{"N3_CDESP" , Posicione("SNG",1,xFilial("SNG")+Padl(aDados[i,5],4,"0"),"NG_CCDEPR") ,NIL},;
			{"N3_CCORREC" , "" ,NIL},;
			{"N3_CCUSTO" , "" ,NIL},;
			{"N3_DINDEPR" , RetDinDepr(StoD(Substr(aDados[i,6],7,4)+Substr(aDados[i,6],4,2)+Substr(aDados[i,6],1,2))) ,NIL},;
			{"N3_VORIG1" , Val(Strtran(aDados[i,7],".","")) ,NIL},;
			{"N3_TXDEPR1" , Posicione("SNG",1,xFilial("SNG")+Padl(aDados[i,5],4,"0"),"NG_TXDEPR1") ,NIL},;
			{"N3_DESCEST" , aDados[i,4] ,NIL},;
			{"N3_VRDACM1" , Val(Strtran(aDados[i,8],".","")) ,NIL}})
